...
@register
def weight(mode, k=0.5):#生成剪枝后的权值pruned_w(numpy类型)和面具mask(numpy类型)

  if mode == "standard":

    def prune(weight_dict, weight_key):#weight_dict整个网络的权值（dict），weight_key是某层的名称（string），对一层网络剪枝
...
def prune_weights(prune_fn,
                  ...
                  louizos_masks=None,
                  smallify_masks=None,
                  hparams=None):#prune_fn为剪枝函数，对应上面的weight方法，weights为numpy类型，为剪枝前的权值，函数返回：剪枝后的整个网络的权值，剪枝数
  weights_pruned = {}
...
def is_prunable_weight(weight):#判断是否是可以剪枝的层
...
def get_current_weights(sess):#获得当前网络的权值，按层类区分
  weights = {}
  variables = {}
